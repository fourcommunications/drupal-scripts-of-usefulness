...
echo ---
cd "$PATHTOWEBROOT" && mv "$ARCHIVELOCATION/$ARCHIVENAME.$ARCHIVEEXTENSION" . && tar -xvf "$ARCHIVENAME.$ARCHIVEEXTENSION" && rm *.tar.gz && cd "$ARCHIVENAME" && touch "core/www/sites/$MULTISITENAME/ENVIRONMENT_TYPE_LIVE.txt" && if ! test -d "$PATHTOSETTINGS/privatefiles"; then mv privatefiles "$PATHTOSETTINGS/"; fi && if test -d "$PATHTOSETTINGS/privatefiles"; then rm -rf privatefiles && ln -s "$PATHTOSETTINGS/privatefiles" && chown -h "$USERNAME:www-data" privatefiles;fi && chown -R "www-data:$USERNAME" "$PATHTOSETTINGS/privatefiles/$MULTISITENAME" && chmod -R 0770 "$PATHTOSETTINGS/privatefiles/$MULTISITENAME" && ln -s "$PATHTOSETTINGS/local_databases.php" && chown -h "$USERNAME:www-data" local_databases.php && cd "www/sites/$MULTISITENAME" && ln -s "$FILESPATH" files && cd ../../../scripts-of-usefulness && ./fix-permissions.sh --drupal_user="$USERNAME"
echo ---
